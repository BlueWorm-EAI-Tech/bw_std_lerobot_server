#!/usr/bin/env python3
"""
WebSocket server for SmolVLA model inference.

This server loads a trained SmolVLA model and serves predictions via WebSocket.
It accepts raw observations (including base64-encoded images) and returns action chunks.

Usage:
    python websocket_smolvla_server.py --port 8001 --model_path /path/to/model --task "pick up the red block..."

Example client request:
    {
        "observation": {
            "observation.state": [0.1, 0.2, ...],
            "observation.images.env_cam": "base64_encoded_image_data",
            "observation.images.left_wrist_cam": "base64_encoded_image_data",
            "observation.images.right_wrist_cam": "base64_encoded_image_data"
        },
...
logger = logging.getLogger("websocket_smolvla_server")
...
class SmolVLAWebSocketServer:
    ...
    def __init__(
        ...
        """Initialize the SmolVLA WebSocket server.
        ...
        Args:
            ...
            task: Default task description for SmolVLA (can be overridden by client)
        """
        ...
        try:
            async for message in websocket:
                try:
                    ...
                    # Get task - can be from request or default
                    task = request.get("task", self.default_task)
                    ...
                    # Add task to observation (SmolVLA requires task input)
                    if task:
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="SmolVLA Model WebSocket Server")
    parser.add_argument(
        "--port",
        type=int,
        default=8001,
    ...
    parser.add_argument(
        ...
        help="Default task description for SmolVLA model"
    )
    parser.add_argument(
        "--log_level",
        type=str,
