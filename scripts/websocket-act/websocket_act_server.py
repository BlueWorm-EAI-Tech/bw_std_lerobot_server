...
This server loads a trained ACT model and serves predictions via WebSocket.
It accepts raw observations (including base64-encoded images) and returns action chunks.

Usage:
    python websocket_act_server.py --port 8000 --model_path /path/to/model --device cuda
...
class ACTWebSocketServer:
    ...
    def __init__(
        ...
        """Initialize the ACT WebSocket server.
        ...
        logger.info(f"Initializing ACT WebSocket Server")
    ...
    def parse_observation(self, obs_data: dict[str, Any]) -> dict[str, torch.Tensor]:
        ...
        # Process environment state if present
        if "observation.environment_state" in obs_data:
            env_state = obs_data["observation.environment_state"]
            if isinstance(env_state, list):
                env_state = torch.tensor(env_state, dtype=torch.float32)
            observation["observation.environment_state"] = env_state
        ...
        try:
            async for message in websocket:
                try:
                    ...
                    # Postprocess each action in the chunk
    ...
    async def start(self):
        ...
        async with websockets.serve(
            ...
            logger.info(f"Server is running and accepting connections")
            await asyncio.Future()  # Run forever


def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="ACT Model WebSocket Server")
    parser.add_argument(
        "--port",
        type=int,
        default=8000,
    ...
    parser.add_argument(
        "--model_path",
        type=str,
        default="/home/lcjs-szw/repos/lerobot/outputs/train/act_your_dataset_20260117/checkpoints/last/pretrained_model",
        help="Path to the pretrained ACT model directory"
    )
    parser.add_argument(
        "--device",
        type=str,
