#!/usr/bin/env python3
"""
Example WebSocket client for testing the SmolVLA model server.

This client demonstrates how to:
1. Connect to the WebSocket server
2. Send observations (with images and task)
3. Receive action chunks
4. Handle responses

Usage:
    python websocket_smolvla_client_example.py --server_url ws://localhost:8001

Example with custom task:
    python websocket_smolvla_client_example.py --server_url ws://localhost:8001 --task "pick up the red block"
"""
...
logger = logging.getLogger("websocket_smolvla_client")


class SmolVLAWebSocketClient:
    """Example WebSocket client for SmolVLA model server."""

    def __init__(self, server_url: str = "ws://localhost:8001", task: str = None):
        """Initialize the client.

        Args:
            server_url: WebSocket server URL
            task: Default task description (can be overridden per request)
        """
    ...
    def create_dummy_observation(
        self,
        state_dim: int = 16,  # Mantis uses 16 DOF
        ...
        # Default camera configurations for Mantis/SmolVLA
    ...
    async def send_observation(
        ...
        Args:
            ...
            task: Task description (uses default if not provided)
        ...
        # Add task if provided or use default
        task_to_use = task or self.default_task
        if task_to_use:
            request["task"] = task_to_use
    ...
    async def run_test_sequence(
        ...
        logger.info(f"State dim: {state_dim}, Task: {task or self.default_task}")
...
async def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="SmolVLA Model WebSocket Client Example")
    parser.add_argument(
        "--server_url",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Task description for SmolVLA"
    )
    parser.add_argument(
        "--cameras",
        type=str,
        default="env_cam:240x320,left_wrist_cam:240x424,right_wrist_cam:240x424",
        help="Camera configurations as name:HxW,name:HxW,..."
    )
    parser.add_argument(
        "--log_level",
        type=str,
