...
class PI05WebSocketServer:
    ...
    async def setup(self):
        ...
        preprocessor_step_names = self._saved_processor_step_names("policy_preprocessor.json")
        if "relative_actions_processor" in preprocessor_step_names:
            preprocessor_overrides["relative_actions_processor"] = {
                "enabled": relative_actions_enabled,
                "exclude_joints": relative_exclude_joints,
                "action_names": action_feature_names,
            }
        elif "delta_actions_processor" in preprocessor_step_names:
            preprocessor_overrides["delta_actions_processor"] = {
                "enabled": relative_actions_enabled,
                "exclude_joints": relative_exclude_joints,
                "action_feature_names": action_feature_names,
            }
        else:
            logger.info("Skipping relative action override: step is not present in saved preprocessor")
        if "absolute_actions_processor" in self._saved_processor_step_names("policy_postprocessor.json"):
            postprocessor_overrides["absolute_actions_processor"] = {
                "enabled": relative_actions_enabled,
                "exclude_joints": relative_exclude_joints,
                "action_names": action_feature_names,
            }
        else:
            logger.info("Skipping absolute_actions_processor override: step is not present in saved postprocessor")
    ...
    def validate_image_keys(self, observation: dict[str, Any]) -> None:
        """Fail fast with an actionable message when image keys do not match the checkpoint."""
        expected = self.expected_image_keys()
        if not expected:
            return
        ...
        raise KeyError(
            "PI05 image key mismatch before policy inference. "
    ...
    def _saved_processor_step_names(self, filename: str) -> set[str]:
        ...
        if not path.exists():
            logger.warning("Processor config %s does not exist", path)
            return set()
        ...
        for step in data.get("steps", []):
            ...
            if registry_name:
                names.add(str(registry_name))
            class_name = step.get("class")
            if class_name:
                names.add(str(class_name).rsplit(".", 1)[-1])
        return names
